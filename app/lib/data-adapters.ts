...
/**
 * Generate mock data for testing and demo purposes
 */
export function generateMockData(
  schema: Array<{ name: string; type: 'string' | 'number' | 'boolean' | 'date' }>,
  count: number = 10
): Record<string, any>[] {
  const data: Record<string, any>[] = [];

  // Demo categories
  const categories = ['Electronics', 'Clothing', 'Furniture', 'Food', 'Books'];
      ...
      switch (field.type) {
        case 'string':
          if (field.name.toLowerCase().includes('category')) {
            row[field.name] = categories[Math.floor(Math.random() * categories.length)];
          } else if (field.name.toLowerCase().includes('region')) {
            row[field.name] = regions[Math.floor(Math.random() * regions.length)];
          } else if (field.name.toLowerCase().includes('product')) {
            row[field.name] = products[Math.floor(Math.random() * products.length)];
          } else if (field.name.toLowerCase().includes('month')) {
            row[field.name] = months[Math.floor(Math.random() * months.length)];
          } else {
        ...
        case 'number':
          if (field.name.toLowerCase().includes('id')) {
            ...
            row[field.name] = Math.round(Math.random() * 100);
          }
          break;
        case 'boolean':
